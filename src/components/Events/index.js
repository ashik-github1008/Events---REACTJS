...
class Events extends Component {
  ...
  onClickEventImg = registrationStatus => {
    ...
    console.log('onClickEventImg')
    this.setState({registerStatus: registrationStatus})
  }
  ...
  render() {
    ...
    return (
      <div className="app-container">
        <nav className="events-container">
          <h1 className="events-heading mb-4">Events</h1>
          <ul className="event-list-container">
            {eventsList.map(eachEvent => (
              <EventItem
                key={eachEvent.id}
                eventDetails={eachEvent}
                onClickEventImg={this.onClickEventImg}
              />
            ))}
          </ul>
        </nav>
        <ActiveEventRegistrationDetails registerStatus={registerStatus} />
      </div>
    )
  }
}
